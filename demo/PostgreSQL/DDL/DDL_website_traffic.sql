...
CREATE TABLE public.visitors (
    ...
    first_visit_time timestamp(6) NULL DEFAULT '1970-01-01 00:00:01'::timestamp without time zone, -- 首次访问时间
    last_visit_time timestamp NULL, -- 最近一次访问时间
...
CREATE INDEX visitors_first_visit_time_idx ON public.visitors USING brin (first_visit_time);
CREATE INDEX visitors_last_visit_time_idx ON public.visitors USING brin (last_visit_time);
...
COMMENT ON COLUMN public.visitors.last_visit_time IS '最近一次访问时间';
...
COMMENT ON COLUMN public.visitors._created_at IS '创建时间';
COMMENT ON COLUMN public.visitors._updated_at IS '更新时间';
...
CREATE TABLE public.sessions (
    ...
    browser_language varchar(255) NULL, -- 浏览器语言
    ...
    enhanced_traffic_group varchar(255) NULL, -- 增强流量渠道分组
    first_event_id varchar(255) NULL, -- 首个事件ID
    ...
    last_event_id varchar(255) NULL, -- 最后一个事件ID
    last_visit_time timestamp(6) NULL, -- 最近一次访问时间
    ...
    referrer_host_sld varchar(255) NULL, -- 前向域名的二级域名
    resolution varchar(255) NULL, -- 屏幕分辨率
    screen_height int2 NULL, -- 屏幕高度
    screen_width int2 NULL, -- 屏幕宽度
    ...
    wx_share_from varchar(255) NULL, -- 微信分享来源
    "_created_at" timestamp(6) NULL DEFAULT now(),
    "_updated_at" timestamp(6) NULL DEFAULT now(),
...
COMMENT ON COLUMN public.sessions.browser_language IS '浏览器语言';
...
COMMENT ON COLUMN public.sessions.enhanced_traffic_group IS '增强流量渠道分组';
COMMENT ON COLUMN public.sessions.first_event_id IS '首个事件ID';
...
COMMENT ON COLUMN public.sessions.last_event_id IS '最后一个事件ID';
COMMENT ON COLUMN public.sessions.last_visit_time IS '最近一次访问时间';
...
COMMENT ON COLUMN public.sessions.referrer_host_sld IS '前向域名的二级域名';
COMMENT ON COLUMN public.sessions.resolution IS '屏幕分辨率';
COMMENT ON COLUMN public.sessions.screen_height IS '屏幕高度';
COMMENT ON COLUMN public.sessions.screen_width IS '屏幕宽度';
...
COMMENT ON COLUMN public.sessions.wx_share_from IS '微信分享来源';
COMMENT ON COLUMN public.sessions._created_at IS '创建时间';
COMMENT ON COLUMN public.sessions._updated_at IS '更新时间';
...
CREATE TABLE public.events (
    ...
    country varchar(255) NULL, -- 国家
    device_type varchar(255) NULL, -- 设备类型
    duration int4 NULL, -- 时长
    enhanced_event_list _text NULL, -- 增强型事件列表
    enhanced_traffic_group varchar(255) NULL, -- 增强流量渠道分组
    hmci varchar(255) NULL, -- 创意参数，对应utm_content
    hmcu varchar(255) NULL, -- 单元名称参数，对应utm_campaign
    hmkw varchar(255) NULL, -- 关键词参数，对应utm_term
    ...
    is_first_day bool NULL, -- 是否首日访问
    is_first_time bool NULL, -- 是否首次触发事件
    is_session_end bool NULL, -- 是否会话结束事件
    is_session_start BOOL NULL, -- 是否会话开始事件
    latest_landing_page text NULL, -- 最近一次落地页
    latest_referrer text NULL, -- 最近一次站外地址
    latest_referrer_host varchar(255) NULL, -- 最近一次站外域名
    latest_referrer_host_sld varchar(255) NULL, -- 最近一次站外域名的二级域名
    ...
    screen_height int2 NULL, -- 屏幕高度
    screen_width int2 NULL, -- 屏幕宽度
    session_start_time timestamp(6) NULL, -- 会话开始时间
    traffic_source_type varchar(255) NULL, -- 流量来源类型
    url text NULL, -- 页面地址
    url_full_path varchar(255) NULL, -- 页面完整路径
    url_host varchar(255) NULL, -- 页面地址域名
    url_host_sld varchar(255) NULL, -- 页面地址的二级域名
...
CREATE INDEX events_session_start_time_idx ON public.events USING brin (session_start_time);
...
COMMENT ON COLUMN public.events.browser_language IS '浏览器语言';
...
COMMENT ON COLUMN public.events.device_type IS '设备类型';
COMMENT ON COLUMN public.events.duration IS '时长';
COMMENT ON COLUMN public.events.enhanced_event_list IS '增强型事件列表';
COMMENT ON COLUMN public.events.enhanced_traffic_group IS '增强流量渠道分组';
...
COMMENT ON COLUMN public.events.is_first_day IS '是否首日访问';
COMMENT ON COLUMN public.events.is_first_time IS '是否首次触发事件';
COMMENT ON COLUMN public.events.is_session_end IS '是否会话结束事件';
COMMENT ON COLUMN public.events.is_session_start IS '是否会话开始事件';
...
COMMENT ON COLUMN public.events.latest_referrer_host_sld IS '最近一次站外域名的二级域名';
...
COMMENT ON COLUMN public.events.referrer_host_sld IS '前向域名的二级域名';
COMMENT ON COLUMN public.events.resolution IS '屏幕分辨率';
COMMENT ON COLUMN public.events.screen_height IS '屏幕高度';
COMMENT ON COLUMN public.events.screen_width IS '屏幕宽度';
COMMENT ON COLUMN public.events.session_start_time IS '会话开始时间';
...
COMMENT ON COLUMN public.events.url_host_sld IS '页面地址的二级域名';
...
COMMENT ON COLUMN public.events.visitor_type IS '访客类型（0: 新访客, 1: 老访客）';
COMMENT ON COLUMN public.events.wx_share_from IS '微信分享来源';
COMMENT ON COLUMN public.events._created_at IS '创建时间';
COMMENT ON COLUMN public.events._updated_at IS '更新时间';
